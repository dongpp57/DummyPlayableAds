/**
 * Main entry — Dummy Playable Ad Scenario S1 V1
 * Pillar: Moment điểm thưởng phạt | Win: Ăn cây mở (+50)
 * Canvas: 640x1136 | 1v1 layout
 *
 * Cây mở 5♣ ở discard pile. Player tap 4♣ + 6♣ → meld [4♣,5♣,6♣] → +50 → RUMMY!
 */
...
import { getInitialHand, getCayMo, MELD_CARD_INDICES } from './card-data-s1-v1.js';
import {
...
async function startGame() {
  ...
  title.text = 'Tap to grab the open card!';
  ...
  // --- Bot avatar + face-down opponent hand ---
  ...
  // --- Center zone: deck + cay_mo (left side, no discard placeholder) ---
  ...
  // --- Cay mo card (open card at center) ---
  ...
  // Glow ring around cay_mo
  const cayMoGlow = new Graphics();
  cayMoGlow.roundRect(centerZone.cayMo.x - 6, centerZone.cayMo.y - 6, DECK_SCALED_W + 12, CARD_HEIGHT * DECK_SCALE + 12, 12);
  ...
  openLabel.x = centerZone.cayMo.x + DECK_SCALED_W / 2;
  ...
  // --- State ---
  // Phase 0: hint user to tap cay_mo (5♣) first
  // Phase 1: cay_mo tapped → all hand cards light up, pointer sweeps across hand, Meld button shown disabled
  // Phase 2: user picks meld cards (4♣, 6♣). Valid picks lift up. When both picked → Meld button enabled
  ...
  // --- Phase 0 setup: cay_mo glows + tappable, hand cards all dim ---
  cards.forEach(c => {
    c.tint = 0x888888;
    c.eventMode = 'none';
  });
  ...
  function triggerMeld() {
    ...
    // Find the picked meld cards by data (value '4' = left, '6' = right)
    const meldCards = cards.filter((c) => c._isMeldCard);
    const card4 = meldCards.find((c) => handData[c._origIdx].value === '4');
    ...
    app.stage.addChild(card4);       // 4♣ left (back)
    app.stage.addChild(cayMoCard);   // 5♣ middle
    app.stage.addChild(card6);       // 6♣ right (front)

    // 4♣ → slot 0
    tweenTo(card4, meldSlots[0].x, meldSlots[0].y, 500);
    ...
    // 6♣ → slot 2
    tweenTo(card6, meldSlots[2].x, meldSlots[2].y, 500);
    ...
    setTimeout(() => {
      ...
      // Floating +50 text right above the meld
      const plus50 = new Text({
        text: '+50',
        style: {
          ...
          fontWeight: 'bold',
          fill: '#FFD700',
          stroke: { color: '#CC0000', width: 8 },
          dropShadow: { color: '#000', blur: 6, distance: 4, alpha: 0.7 },
        },
      });
      plus50.anchor.set(0.5, 1);
      const meldCenterX = meldSlots[1].x + MELD_SCALED_W / 2;
      const startY = meldSlots[0].y - 10;
      plus50.x = meldCenterX;
      plus50.y = startY;
      plus50.scale.set(0);
      app.stage.addChild(plus50);

      // Pop in (scale 0 → 1.2 → 1) then float up + fade
      ...
      const tick = () => {
        ...
        if (el < popDur) {
          ...
          plus50.scale.set(1.2 - (el - popDur) / 100 * 0.2);
        } else if (el < popDur + 100 + floatDur) {
          plus50.scale.set(1);
          const t = (el - popDur - 100) / floatDur;
          plus50.y = startY - t * 80;
          plus50.alpha = 1 - t;
        } else {
          plus50.parent?.removeChild(plus50);
          return;
        }
        requestAnimationFrame(tick);
      };
    ...
    setTimeout(() => { stopBgm(); openUrl(STORE_URL); }, 5000);
  }
  ...
  // --- Initial pointer: hint cay_mo ---
  title.text = 'Tap the open card!';
