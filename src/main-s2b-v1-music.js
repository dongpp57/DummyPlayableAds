/**
 * Main entry — Dummy Playable Ad Scenario S2b V1 (Watch the trap / auto-play demo)
 * Pillar: Moment điểm thưởng phạt | LOSE demo (giáo dục, không trừng phạt)
 * Canvas: 640x1136 | 1v1 layout (player + 1 bot)
 *
 * User CHỈ XEM. Title: "Watch out for this trap!"
 * Auto-play 4-5s: 1 lá tự discard → opponent reveal lurk → meld → "OPPONENT WINS THIS!" + ❌
 * → Title "Don't fall for it — Play smart!" → CTA "PLAY NOW" pulse.
 */
...
// S2b cards: 2 hand + 3 player meld + 1 cay_mo + 2 bot lurk = 8 unique
...
async function startGame() {
  ...
  title.text = 'Watch out for this trap!';
  ...
  for (let i = 0; i < handData.length; i++) {
    ...
    // S2b: NO interaction — user just watches
    card.eventMode = 'none';
  ...
  // S2b: NO discard button, NO tap interaction, NO pointer cycle.
  // Auto-play timeline starts after a brief intro.
  // 1.5s: highlight both hand cards as "trap candidates" (warning glow)
  // 3.0s: pick one card automatically → lift it → trigger commitDiscard()
  setTimeout(() => {
    // Pick one card (random) to auto-discard
    const pickIdx = Math.random() < 0.5 ? 0 : 1;
    selectedCard = cards[pickIdx];
    if (selectedCard._origX === undefined) {
      selectedCard._origX = selectedCard.x;
      selectedCard._origY = selectedCard.y;
    }
    unlockAudio();
    playBgm('bgm', 0.8);
    highlightCard(selectedCard, true);
    play('card');
    tweenTo(selectedCard, selectedCard._origX, selectedCard._origY - 28, 250);
    title.text = 'Auto-discarding...';
  }, 1800);
